...
# step2-4_helper-script.sh
# Helper script for individual sequences dotplot generation
...
# Function to get all individual FASTA files (excluding all_*.fasta)
get_individual_fasta_files() {
    ...
    find "$extracted_dir" -name "*.fa" -o -name "*.fasta" | grep -v "all_" | sort
}

# Function to count individual FASTA files
count_individual_fasta_files() {
    local extracted_dir="$1"
    get_individual_fasta_files "$extracted_dir" | wc -l
}

# Function to generate summary statistics
generate_summary() {
    local output_dir="$1"
    local summary_file="${output_dir}/individual_dotplot_generation_summary.txt"
    ...
    local alternate_files=$(find "$output_dir" -name "*alternate_hits*.pdf" | wc -l)
    
    # Count by sample
    local unique_samples=$(find "$output_dir" -name "*.pdf" | sed 's/.*\///' | cut -d'_' -f1-2 | sort -u | wc -l)
    
    # Write summary
    cat > "$summary_file" << EOF
Individual Sequences Dotplot Generation Summary
==============================================
Generated on: $(date)

Total dotplots generated: $total_files
Failed dotplots: $failed_files
Unique samples processed: $unique_samples
...
- Alternate hits: $alternate_files
...
create_output_structure() {
    ...
    mkdir -p "${output_dir}/mVenus/alternate_hits"
    mkdir -p "${output_dir}/CodA/primary_hits"
    mkdir -p "${output_dir}/CodA/combined_hits"
    mkdir -p "${output_dir}/CodA/alternate_hits"
...
organize_dotplots() {
    ...
    if [ -d "${output_dir}/mVenus" ]; then
        ...
        find "$output_dir" -name "*mVenus*alternate_hits*.pdf" -exec mv {} "${output_dir}/mVenus/alternate_hits/" \;
    fi
    ...
    if [ -d "${output_dir}/CodA" ]; then
        ...
        find "$output_dir" -name "*CodA*alternate_hits*.pdf" -exec mv {} "${output_dir}/CodA/alternate_hits/" \;
    fi
...
export -f generate_dotplot get_individual_fasta_files count_individual_fasta_files
export -f generate_summary create_output_structure organize_dotplots
export -f run_single_dotplot

log_info "step2-4_helper-script.sh loaded successfully"
