...
workflow PIPELINE {
    ...
    //
    // SUBWORKFLOW: Run genome assembly with HiFiASM
    //
    ASSEMBLY (
        ch_samplesheet
    )
    ...
    emit:
    primary_assemblies  = ASSEMBLY.out.primary_fasta    // channel: [ val(meta), path(fa) ]
    alternate_assemblies = ASSEMBLY.out.alternate_fasta // channel: [ val(meta), path(fa) ]
    assembly_logs       = ASSEMBLY.out.assembly_log     // channel: [ val(meta), path(log) ]
